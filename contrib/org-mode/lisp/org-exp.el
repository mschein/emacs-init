	...
	(setq rtn (with-temp-buffer
      ...
      (setq rtn
		...
		(flet ((make-option-string
	     ...
	     ;; XXXX Mondo hack here.... this isn't how the experimental org export does it.
	     ((eq org-export-current-backend 'linkedin-wiki)
              (setq rtn (org-export-number-lines rtn 0 0 num cont rpllbl fmt))
              (concat caption (format "{code}%s{code}" rtn)))
